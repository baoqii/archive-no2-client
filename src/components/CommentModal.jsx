...
const CommentForm = ({ onSubmit, onClose, setUpdateCommentsSection }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!author || !text) {
      setErrorMessage("Author and comment fields are required.");
      return;
    }
  ...
  return (
    ...
    >
      {errorMessage && (
        <div className="error-message bg-red-100 border border-red-400 text-red-700 hover:text-red-900 active:text-red-800 px-4 py-3 relative font-semibold tracking-wide font-lato">
          {errorMessage}
          <button
      ...
      <div className="flex flex-col px-6 py-5 bg-wild-sand-50 dark:bg-mine-shaft-950">
        <div className="w-full">
          ...
          <input
            type="text"
            className="w-full px-5 py-3 bg-white dark:bg-eerie-black-950 border border-alto-200 dark:border-onyx-900 rounded shadow-s appearance-none"
            value={author}
        ...
        <textarea
          value={text}
          className="px-5 py-3 mb-5 bg-white dark:bg-eerie-black-950 border border-alto-200 dark:border-onyx-900 rounded shadow-sm h-36"
